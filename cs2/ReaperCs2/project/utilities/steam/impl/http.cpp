...
namespace steam {
	...
	namespace detail {
		...
		static bool wait_for_call (std::uintptr_t call, std::uint32_t timeout_ms, int* out_status_code) {
			if (out_status_code) {
				*out_status_code = 0;
			}
			...
			while (true) {
				...
				if (memory::call<bool> (MODULE_EXPORT ("steam_api64.dll:SteamAPI_ISteamUtils_IsAPICallCompleted"), utils, call, &failed)) {
					...
					if (got_result && !failed && out_status_code) {
						*out_status_code = result.m_status_code;
					}

					return got_result && !failed && result.m_request_successful && result.m_status_code >= 200 && result.m_status_code < 300;
				}
	...
	bool http::send_and_wait (std::uint32_t req, std::uint32_t timeout_ms, int* out_status_code) {
		auto call {0ull};
		...
		return detail::wait_for_call (call, timeout_ms, out_status_code);
	}
