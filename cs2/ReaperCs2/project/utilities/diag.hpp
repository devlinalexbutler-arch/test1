...
#pragma pack( pop )
	...
	inline void set_module( HMODULE module_handle )
	{
		...
		{
			// A manually mapped DLL is not registered with the Windows loader,
			// so GetModuleFileNameW cannot resolve its path.  Preserve diagnostics
			// by writing the artifacts to the per-user temporary directory instead.
			const auto temp_length = GetTempPathW( MAX_PATH, directory );
