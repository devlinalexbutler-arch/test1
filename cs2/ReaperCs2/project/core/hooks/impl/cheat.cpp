...
namespace hooks {
	...
	void __fastcall cheat::frame_stage_notify( std::uintptr_t thisptr, int stage )
	{
		...
		// Apply the local spin pose after Source 2 has finished this stage's
		// animation work; doing it before the original call is overwritten.
		if ( stage == 7 && systems::g_local.get( ).is_valid( ) && systems::g_view.has_camera( ) )
		{
			features::combat::g_misc.antiaim( ).apply_model_spin( );
		}
	...
	void __fastcall cheat::create_move( std::uintptr_t thisptr, int slot, bool active )
	{
		...
		systems::g_input.update( );
		{
...

			{
				...
				diag::set_exception_phase( "create_move: spinbot" );
			...
			if ( final_base && final_base->subtick_moves_size( ) > 0
				...
				&& !settings::g_movement.auto_strafe.value )
			{
				final_base->set_forwardmove( 0.0f );
				final_base->set_leftmove( 0.0f );
			}

			diag::set_exception_phase( "create_move: final shot commit" );
			features::combat::g_rage.finalize_shot_command( current_cmd );
	...
	{
		const auto flash_percent = std::clamp( settings::g_misc.m_removals.flash_alpha.value, 0.0f, 100.0f );
		const auto view_pawn = systems::g_local.get( ).view_pawn( );
		if ( view_pawn )
		{
			const auto max_alpha = flash_percent / 100.0f * 255.0f;
			memory::write<float>( view_pawn + SCHEMA( "C_CSPlayerPawnBase", "m_flFlashMaxAlpha"_hash ), max_alpha );
		}
