...
namespace features::movement {
	...
	namespace {
		...
		[[nodiscard]] float angle_distance( float lhs, float rhs )
		{
			auto delta = lhs - rhs;
			math::helpers::normalize_angle( delta );
			return std::fabsf( delta );
		}
		...
		[[nodiscard]] float optimized_wish_yaw( float vel_x, float vel_y, float target_yaw, float frame_time, bool prefer_positive, float friction, float wishspeed, float air_accel, float air_max_wishspeed )
		{
			...
			const auto velocity_yaw = std::atan2f( vel_y, vel_x ) * ( 180.0f / std::numbers::pi_v<float> );
			const auto ideal = ref_ideal_angle( speed, frame_time, wishspeed, air_accel, air_max_wishspeed );
			...
			const auto evaluate = [&]( float direction )
			{
				candidate result{};
				result.yaw = velocity_yaw + ideal * direction;
				math::helpers::normalize_angle( result.yaw );

				auto next_x = vel_x;
				auto next_y = vel_y;
				ref_air_accel_sim( next_x, next_y, result.yaw, frame_time, friction, wishspeed, air_accel, air_max_wishspeed );

				result.speed_squared = next_x * next_x + next_y * next_y;
				const auto next_yaw = std::atan2f( next_y, next_x ) * ( 180.0f / std::numbers::pi_v<float> );
				result.heading_error = angle_distance( next_yaw, target_yaw );
				return result;
			};

			const auto positive = evaluate( 1.0f );
			const auto negative = evaluate( -1.0f );
			constexpr auto speed_epsilon = 0.0001f;

			if ( positive.speed_squared > negative.speed_squared + speed_epsilon )
			{
				return positive.yaw;
			}

			if ( negative.speed_squared > positive.speed_squared + speed_epsilon )
			{
				return negative.yaw;
			}

			if ( positive.heading_error < negative.heading_error )
			{
				return positive.yaw;
			}

			if ( negative.heading_error < positive.heading_error )
			{
				return negative.yaw;
			}

			return prefer_positive ? positive.yaw : negative.yaw;
		}
	...
	void test_strafer::quantized_path( systems::input::usercmd* cmd )
	{
		...
		const auto sub_frame = cstypes::tick_interval * when_step;
		...
		{
			const auto prefer_positive = ( ( this->m_substep_counter + i ) % 2 ) == 0;
			const auto wishdir_yaw = optimized_wish_yaw( sim_vx, sim_vy, target_yaw, sub_frame, prefer_positive, surface_friction, sv_maxspeed, sv_airaccelerate, sv_air_max_wishspeed );
