...
namespace features::combat {
	...
	bool shared::lagcomp::record::setup( std::uintptr_t pawn )
	{
		...
		this->eye_angles = memory::read<math::vector3>( pawn + SCHEMA( "C_CSPlayerPawn", "m_angEyeAngles"_hash ) );
	...
	float shared::calculate_hitchance( const math::vector3& shoot_position, const math::vector3& aim_angle, const systems::hitboxes::entry& hitbox, const systems::bones::data& bone, float inaccuracy, float spread, int samples ) const
	{
		...
		struct spread_cache
		{
			...
			int count{};
			bool initialized{};
			std::array<math::vector2, 384> values{};
		};
	...
	std::optional<math::vector3> shared::find_spread_correction( const math::vector3& aim_angle, int tick ) const
	{
