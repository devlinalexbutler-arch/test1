...
namespace features::combat {
	namespace {
		...
		[[nodiscard]] bool doubletap_offline_test_session( )
		{
			...
			const auto sv_cheats = addresses::globals::cvar->find( "sv_cheats"_hash );
			return sv_cheats && sv_cheats->get<bool>( );
		}
	...
	bool rage::process_doubletap( systems::input::usercmd* cmd, const systems::local::snapshot& local, bool charge_dt )
	{
		...
		if ( !config.doubletap.value || !charge_dt || !doubletap_offline_test_session( ) )
		{
			return false;
		}
	...
	std::optional<rage::stop_prediction> rage::predict_stop( const aim_context& ctx, const math::vector3& current_eye, const systems::local::snapshot& local ) const
	{
		...
		for ( auto i = 0; i < 15; ++i )
		{
			...
			auto next_vel = sim_vel;
			if ( new_speed > 0.0f )
				next_vel *= ( new_speed / sim_speed );
			else
				next_vel = {};

			displacement += ( sim_vel + next_vel ) * ( cstypes::tick_interval * 0.5f );
			sim_vel = next_vel;
			if ( new_speed <= 0.0f )
				break;
		}
	...
	{
		if ( !settings::g_combat.m_ragebot.enabled.value )
		{
			return;
		}
		...
		if ( duckpeek_active )
		{
			best = this->select_best( ctx, current_hits, standing_inaccuracy );
		}
		...
		if ( !shot_viable && this->should_stop_movement( ctx ) )
		{
			...
			if ( stop )
			{
				...
				const auto planned_accurate = planned.valid && planned.hitchance >= needed_hc;
				const auto planned_force = planned.valid && config.force_shot.value &&
					g_shared.is_max_accuracy( stop->inaccuracy );
				this->m_should_stop = planned_accurate || planned_force;
			}
			else
			{
	...
	{
		if ( !settings::g_combat.m_zeusbot.enabled.value )
		{
			return;
		}
	...
	{
		if ( !settings::g_combat.m_knifebot.enabled.value )
		{
			return;
		}
		...
		(void)this->fire_melee( cmd, chosen, local );
	}
	...
	std::vector<rage::scan_hit> rage::scan_player( const math::vector3& eye, float inaccuracy, const aim_context& ctx, candidate& cand, shared::lagcomp::record* record, const systems::local::snapshot& local ) const
	{
		...
		std::vector<debug_point> pending_debug_points;
		if ( config.debug_multipoints.value )
		{
			pending_debug_points.reserve( static_cast< std::size_t >( scan_count ) * 12 );
		}
		...
		for ( auto idx = 0; idx < scan_count; ++idx )
		{
			...
			if ( config.debug_multipoints.value )
			{
				pending_debug_points.push_back( { center, hitbox_index, true } );
			}
			...
			if ( config.pointscale > 0.0f )
			{
				...
				for ( const auto& mp : mps )
				{
					...
					if ( config.debug_multipoints.value )
					{
						pending_debug_points.push_back( { mp, hitbox_index, false } );
					}
				}
			}
		}
		...
		if ( !pending_debug_points.empty( ) )
		{
			std::lock_guard lock( m_debug_mtx );
			m_debug_points.insert( m_debug_points.end( ), pending_debug_points.begin( ), pending_debug_points.end( ) );
		}
		...
		for ( const auto& tp : points )
		{
			...
			const auto intended_hitgroup = systems::g_hitboxes.hitgroup_from_hitbox( tp.hitbox_index );
			if ( intended_hitgroup > 0 && pen.hitgroup != intended_hitgroup )
			{
				continue;
			}
	...
	rage::target rage::select_best( const aim_context& aim_ctx, const std::vector<scan_hit>& hits, float eval_inaccuracy ) const
	{
		...
		auto cheap_score = [ & ]( const scan_hit& h ) -> float
			{
				const auto lethal_bonus = h.damage >= static_cast< float >( h.health ) ? 150000.0f : 0.0f;
				const auto near_lethal = ( !lethal_bonus && h.damage >= static_cast< float >( h.health ) * 0.85f ) ? 25000.0f : 0.0f;
				const auto direct_bonus = h.penetrated ? 0.0f : 8000.0f;
				const auto center_bonus = h.is_center ? 3500.0f : 0.0f;
				// Prefer head / upper body harder; still allow body when it's the only lethal path
				const auto hitgroup = static_cast< float >( hitgroup_priority( h.hitbox_index ) );

				return lethal_bonus + near_lethal + direct_bonus + center_bonus +
					h.damage * 28.0f + hitgroup * 18.0f - h.fov * 1.35f;
			};
	...
	float rage::evaluate_hitchance( const scan_hit& hit, const aim_context& ctx, float inaccuracy ) const
	{
		...
		// Extra samples = more accurate HC estimate on edge angles / small hitboxes
		return g_shared.calculate_hitchance( hit.source_eye.position, hit.aim_angle, hit.hitbox, hit.record->bones[ hit.bone_index ], inaccuracy, ctx.spread, 320 );
	}
			...
			for ( auto ri = 0; ri < cand.record_count; ++ri )
			{
				...
				auto backstab{ false };
				{
					...
					if ( dist_2d > 0.001f )
					{
						...
						math::helpers::angle_vectors_left( record->eye_angles, &eye_forward );
	...
	void rage::stage_shot_command( systems::input::usercmd* cmd, int history_index, std::uintptr_t attack_button )
	{
		this->m_shot_commit = {};
		if ( !cmd || !attack_button )
		{
			return;
		}

		auto& commit = this->m_shot_commit;
		commit.command = cmd;
		commit.attack_button = attack_button;
		commit.attack_held = ( cmd->buttons.value & attack_button ) != 0;
		commit.attack_changed = ( cmd->buttons.value_changed & attack_button ) != 0;
		commit.attack_scrolled = ( cmd->buttons.value_scroll & attack_button ) != 0;
		commit.attack1_index = cmd->csgo_user_cmd.attack1_start_history_index( );
		commit.attack2_index = cmd->csgo_user_cmd.attack2_start_history_index( );
		commit.history_index = history_index;

		if ( const auto base = cmd->csgo_user_cmd.mutable_base( ) )
		{
			if ( const auto angles = base->viewangles( ) )
			{
				commit.base_angles = { angles->x( ), angles->y( ), angles->z( ) };
				commit.has_base_angles = true;
			}
		}

		if ( history_index >= 0 && history_index < cmd->csgo_user_cmd.input_history_size( ) )
		{
			if ( const auto entry = cmd->csgo_user_cmd.mutable_input_history( history_index ) )
			{
				if ( const auto angles = entry->view_angles( ) )
				{
					commit.history_angles = { angles->x( ), angles->y( ), angles->z( ) };
					commit.has_history_angles = true;
				}

				commit.has_render_tick = entry->has_render_tick_count( );
				commit.has_render_fraction = entry->has_render_tick_fraction( );
				commit.has_player_tick = entry->has_player_tick_count( );
				commit.has_player_fraction = entry->has_player_tick_fraction( );
				commit.render_tick = entry->render_tick_count( );
				commit.render_fraction = entry->render_tick_fraction( );
				commit.player_tick = entry->player_tick_count( );
				commit.player_fraction = entry->player_tick_fraction( );
			}
		}
	...
	void rage::finalize_shot_command( systems::input::usercmd* cmd )
	{
		auto& commit = this->m_shot_commit;
		if ( !commit.active || !cmd || commit.command != cmd )
		{
			return;
		}

		const auto apply_bit = [ ]( std::uintptr_t& value, std::uintptr_t bit, bool enabled )
		{
			if ( enabled ) value |= bit;
			else value &= ~bit;
		};

		apply_bit( cmd->buttons.value, commit.attack_button, commit.attack_held );
		apply_bit( cmd->buttons.value_changed, commit.attack_button, commit.attack_changed );
		apply_bit( cmd->buttons.value_scroll, commit.attack_button, commit.attack_scrolled );
		cmd->csgo_user_cmd.set_attack1_start_history_index( commit.attack1_index );
		cmd->csgo_user_cmd.set_attack2_start_history_index( commit.attack2_index );

		if ( const auto base = cmd->csgo_user_cmd.mutable_base( ); base && commit.has_base_angles )
		{
			if ( const auto angles = base->mutable_viewangles( ) )
			{
				angles->set_x( commit.base_angles.x );
				angles->set_y( commit.base_angles.y );
				angles->set_z( commit.base_angles.z );
			}
		}

		if ( commit.history_index >= 0 && commit.history_index < cmd->csgo_user_cmd.input_history_size( ) )
		{
			if ( const auto entry = cmd->csgo_user_cmd.mutable_input_history( commit.history_index ) )
			{
				if ( commit.has_history_angles )
				{
					if ( const auto angles = entry->mutable_view_angles( ) )
					{
						angles->set_x( commit.history_angles.x );
						angles->set_y( commit.history_angles.y );
						angles->set_z( commit.history_angles.z );
					}
				}

				if ( commit.has_render_tick ) entry->set_render_tick_count( commit.render_tick );
				if ( commit.has_render_fraction ) entry->set_render_tick_fraction( commit.render_fraction );
				if ( commit.has_player_tick ) entry->set_player_tick_count( commit.player_tick );
				if ( commit.has_player_fraction ) entry->set_player_tick_fraction( commit.player_fraction );
			}
		}
	...
	void rage::fire_gun( systems::input::usercmd* cmd, const target& tgt, bool was_forced, const math::vector3& shoot_eye, const systems::local::snapshot& local, bool subtick_attack )
	{
		...
		if ( !subtick_attack )
		{
			...
			if ( history_size > 0 )
			{
				attack_history_index = history_size - 1;
		...
		// Never rewrite shot angles for AA. hide_shots only skips AA on this tick.
		if ( const auto angles = base->mutable_viewangles( ) )
		{
			angles->set_x( punched_aim.x );
			angles->set_y( punched_aim.y );
			if ( config.no_spread.value )
			{
				angles->set_z( aim_angle.z );
			}
		...
		this->stage_shot_command( cmd, attack_history_index, cstypes::command_buttons::in_attack );
	}
	...
	bool rage::fire_melee( systems::input::usercmd* cmd, const target& tgt, const systems::local::snapshot& local )
	{
		...
		this->stage_shot_command( cmd, history_index, attack_button );
	...
	std::vector<math::vector3> rage::generate_multipoints( const systems::hitboxes::entry& hitbox, const math::vector3& center, const math::quaternion& bone_rot, float pointscale, const math::vector3& shoot_pos, float inaccuracy ) const
	{
		...
		switch ( hitbox.index )
		{
		case 0: // head — denser ring for better edge shots / peek angles
		{
			out.reserve( 8 );
			...
			out.push_back( scaled_surface( -up ) );
			// diagonals
			const auto ur = ( up + right ).normalized( );
			const auto ul = ( up - right ).normalized( );
			const auto dr = ( -up + right ).normalized( );
			const auto dl = ( -up - right ).normalized( );
			out.push_back( scaled_surface( ur ) );
			out.push_back( scaled_surface( ul ) );
			out.push_back( scaled_surface( dr ) );
			out.push_back( scaled_surface( dl ) );
			break;
		}
	...
	{
		const auto configured = override_active ? config.min_damage_override_value : config.min_damage.value;
		const auto hp = static_cast< float >( target_health );
		if ( configured > 100 )
		{
			return hp + static_cast< float >( configured - 100 );
		}

		const auto absolute = static_cast< float >( std::max( configured, 0 ) );
		if ( hp < absolute )
		{
			return hp + 1.0f;
		}
