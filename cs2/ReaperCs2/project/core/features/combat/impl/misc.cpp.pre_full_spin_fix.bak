...
namespace features::combat {
	...
	void misc::antiaim::on_create_move( systems::input::usercmd* cmd )
	{
		...
		const auto spinbot_active = settings::g_combat.m_ragebot.spinbot.value;

		if ( spinbot_active )
		{
			const auto spin_speed = static_cast< float >( std::clamp( settings::g_combat.m_ragebot.spin_speed.value, 1, 64 ) );
			this->m_spin_yaw = math::helpers::normalize_yaw( this->m_spin_yaw + spin_speed );
			yaw = this->m_spin_yaw;

			if ( settings::g_combat.m_ragebot.spin_pitch_down.value )
			{
				pitch = 89.0f;
			}
			else if ( settings::g_combat.m_ragebot.spin_pitch_up.value )
			{
				pitch = -89.0f;
			}
		}
		...
		const auto override_pitch = spinbot_active
			&& ( settings::g_combat.m_ragebot.spin_pitch_down.value
				|| settings::g_combat.m_ragebot.spin_pitch_up.value );
		if ( override_pitch )
		{
			angles->set_x( this->m_modified_angles.x );
		}

		const auto override_yaw = spinbot_active;
		if ( override_yaw )
		{
		...
		// Every subtick carries its own view angle. Stamp the complete history so
		// pitch-down remains authoritative regardless of which entry CS2 consumes.
	...
	void misc::antiaim::apply_model_spin( )
	{
		...
		const auto heading = this->m_modified_angles;
		// Scene-node pitch rotates the entire entity into the floor. Keep the body
		// upright while allowing third-person heading to display the server pitch.
		const auto upright_rotation = math::vector3{ 0.0f, this->m_modified_angles.y, 0.0f };

		const auto put_vec = [ & ]( std::uintptr_t base, int off, const math::vector3& value )
		{
			if ( base && off )
			{
				memory::write<math::vector3>( base + off, value );
			}
		};
		const auto put_yaw = [ & ]( std::uintptr_t base, int off )
		{
			if ( base && off )
			{
				memory::write<float>( base + off, this->m_modified_angles.y );
			}
		};

		// The old third-person heading field no longer exists in the current
		// player schema. Drive the animation's eye-angle interpolation state
		// instead, otherwise the local model keeps interpolating back to level.
		put_vec( local.pawn, SCHEMA( "C_CSPlayerPawn", "m_angEyeAngles"_hash ), heading );
		put_vec( local.pawn, SCHEMA( "C_CSPlayerPawn", "m_angEyeAnglesVelocity"_hash ), math::vector3{} );

		if ( const auto old_eye_angles = SCHEMA( "C_CSPlayerPawn", "m_arrOldEyeAngles"_hash ) )
		{
			for ( auto index = 0; index < 4; ++index )
			{
				memory::write<math::vector3>( local.pawn + old_eye_angles
					+ static_cast< std::uintptr_t >( index ) * sizeof( math::vector3 ), heading );
			}
		}
		put_yaw( local.pawn, SCHEMA( "C_CSPlayerPawnBase", "m_flLowerBodyYawTarget"_hash ) );
		put_yaw( local.pawn, SCHEMA( "C_CSPlayerPawn", "m_flLowerBodyYawTarget"_hash ) );

		const auto node = memory::read<std::uintptr_t>( local.pawn + SCHEMA( "C_BaseEntity", "m_pGameSceneNode"_hash ) );
		put_vec( node, SCHEMA( "CGameSceneNode", "m_angRotation"_hash ), upright_rotation );
		put_vec( node, SCHEMA( "CGameSceneNode", "m_angAbsRotation"_hash ), upright_rotation );
		put_vec( node, SCHEMA( "CGameSceneNode", "m_angWrappedLocalRotation"_hash ), upright_rotation );

		// Eye angles alone do not alter an already evaluated local-player pose.
		// Re-evaluate the skeleton after publishing the silent pitch so the model
		// actually bends up/down instead of remaining level.
		if ( node && ( settings::g_combat.m_ragebot.spin_pitch_down.value
			|| settings::g_combat.m_ragebot.spin_pitch_up.value ) )
		{
			const auto rebuild_skeleton = PATTERN( patterns::game_scene_node_set_skeleton );
			if ( rebuild_skeleton )
			{
				memory::call<void>( rebuild_skeleton, node, 0x100 );
			}

			// Skeleton evaluation may publish the node heading again. Keep the root
			// upright and apply only yaw after the pose has been rebuilt.
			put_vec( node, SCHEMA( "CGameSceneNode", "m_angRotation"_hash ), upright_rotation );
			put_vec( node, SCHEMA( "CGameSceneNode", "m_angAbsRotation"_hash ), upright_rotation );
			put_vec( node, SCHEMA( "CGameSceneNode", "m_angWrappedLocalRotation"_hash ), upright_rotation );
		}
	...
	{
		// Anti-aim removed — permanently disabled.
		return;
