...
namespace features::combat {
	...
	public:
		...
		[[nodiscard]] float calculate_hitchance( const math::vector3& shoot_position, const math::vector3& aim_angle, const systems::hitboxes::entry& hitbox, const systems::bones::data& bone, float inaccuracy, float spread, int samples = 320 ) const;
		[[nodiscard]] std::optional<math::vector3> find_spread_correction( const math::vector3& aim_angle, int tick ) const;
	...
	private:
		...
		public:
			...
			void reapply_movement_correction( systems::input::usercmd* cmd );
			void apply_model_spin( );
			void finalize_spin_angles( systems::input::usercmd* cmd ) const;
			...
			[[nodiscard]] const math::vector3& get_real_angles( ) const { return this->m_old_angles; }
			[[nodiscard]] bool is_active( ) const { return this->m_antiaim_active; }

		private:
			[[nodiscard]] float get_pitch( );
			[[nodiscard]] float get_at_target_yaw( const systems::local::snapshot& local ) const;
			[[nodiscard]] float get_yaw( const systems::local::snapshot& local );
	...
	public:
		...
		void finalize_shot_command( systems::input::usercmd* cmd );
	...
	private:
		...
		[[nodiscard]] bool fire_melee( systems::input::usercmd* cmd, const target& tgt, const systems::local::snapshot& local );
		void stage_shot_command( systems::input::usercmd* cmd, int history_index, std::uintptr_t attack_button );
		...
		struct shot_command_commit
		{
			systems::input::usercmd* command{};
			std::uintptr_t attack_button{};
			bool attack_held{};
			bool attack_changed{};
			bool attack_scrolled{};
			int attack1_index{ -1 };
			int attack2_index{ -1 };
			math::vector3 base_angles{};
			bool has_base_angles{};
			int history_index{ -1 };
			math::vector3 history_angles{};
			bool has_history_angles{};
			int render_tick{};
			float render_fraction{};
			int player_tick{};
			float player_fraction{};
			bool has_render_tick{};
			bool has_render_fraction{};
			bool has_player_tick{};
			bool has_player_fraction{};
			bool active{};
		} m_shot_commit{};
	...
	private:
		...
		enum class rcs_mode : std::uint8_t
		{
			inactive,
			locked,
			standalone
		...
		[[nodiscard]] target_result find_target( const math::vector3& shoot_position, const math::vector3& view_angles, const settings::combat::legitbot::weapon_group& config, const systems::local::snapshot& local );
		[[nodiscard]] scan_point scan_player( std::uintptr_t pawn, int target_health, shared::lagcomp::record* record, const math::vector3& shoot_position, const math::vector3& view_angles, const settings::combat::legitbot::weapon_group& config, const systems::local::snapshot& local ) const;
		...
		[[nodiscard]] math::vector3 corrected_rcs_angle( const math::vector3& base_angle, const math::vector3& punch_delta, int amount, int rand_min, int rand_max ) const;
		void write_rcs_command_angle( systems::input::usercmd* cmd, const math::vector3& angle ) const;
		void update_standalone_rcs( systems::input::usercmd* cmd, const math::vector3& view_angles, const math::vector3& aim_punch, int amount, int rand_min, int rand_max, const systems::local::snapshot& local );
		...
		math::vector3 m_standalone_prev_punch{};
		rcs_mode m_rcs_mode{ rcs_mode::inactive };
		...
		float m_aim_step_x{};
		float m_aim_step_y{};
		std::uintptr_t m_sticky_pawn{};
		std::uintptr_t m_spray_pawn{};
		int m_sticky_ticks{};
		float m_humanize_phase{};

		float m_trigger_delay_start{};
		std::uintptr_t m_trigger_pending_pawn{};
		int m_trigger_pending_hitbox{ -1 };
