...
#include <utilities/presence.hpp>
...
(function () {
	...
	SWeaponManager = (function () {
		...
		function updateNow(xuid, account_id, weapons, active_path, kryptik, agent, skin_count) {
			...
			var badgeId = "kryptik-badge-" + xuid;
			var badge = nameIcons.FindChildTraverse(badgeId);
			if (kryptik) {
				if (!isValid(badge)) {
					badge = $.CreatePanel("Label", nameIcons, badgeId);
					badge.AddClass("kryptik-presence-badge");
					badge.style.fontSize = "11px";
					badge.style.fontWeight = "bold";
					badge.style.color = "#00dc5aff";
					badge.style.marginLeft = "5px";
					badge.style.verticalAlign = "center";
				}
				badge.text = "KRYPTIK";
				badge.SetPanelEvent("onmouseover", function () {
					$.DispatchEvent("DOTAShowTextTooltip", badge, "Agent " + agent + "  |  " + skin_count + " shared skins");
				});
				badge.SetPanelEvent("onmouseout", function () { $.DispatchEvent("DOTAHideTextTooltip", badge); });
				badge.style.visibility = "visible";
			} else if (isValid(badge)) {
		...
		return {
			update: function (xuid, account_id, weapons, active_path, kryptik, agent, skin_count) {
				...
				updateNow(xuid, account_id, weapons, active_path, kryptik, agent, skin_count);
			},
			...
			clear: function () {
				...
				var badges = sb.FindChildrenWithClassTraverse("kryptik-presence-badge");
				for (var j = 0; j < badges.length; ++j) {
					if (isValid(badges[j])) badges[j].style.visibility = "collapse";
				}
			}
		};
	...
	SClient.register_handler("updateWeapons", function (msg) {
		if (msg && msg.content)
			SWeaponManager.update(msg.content.xuid, msg.content.account_id, msg.content.weapons, msg.content.active_path,
				msg.content.kryptik, msg.content.agent, msg.content.skin_count);
	});

	SClient.register_handler("clearWeapons", function (msg) {
		if (msg && msg.content)
			SWeaponManager.update(msg.content.xuid, msg.content.account_id, [], "", false, 0, 0);
	});
	...
	void scoreboard_weapons::on_frame_stage_notify () {
		if (!settings::g_misc.m_scoreboard_weapons.enabled.value && !presence::available()) {
	...
	void scoreboard_weapons::send_player_weapons (
		...
		if ( presence::is_local_user( steamid ) ) {
			state.kryptik = true;
			const auto team = memory::safe_read<int>( controller + SCHEMA( "C_BaseEntity", "m_iTeamNum"_hash ) ).value_or( 0 );
			state.agent = team == 3 ? settings::g_changer.agents.ct_def : settings::g_changer.agents.t_def;
			state.skin_count = settings::g_changer.skins.data.size( );
		}
		else if ( const auto user = presence::find( steamid ) ) {
			state.kryptik = true;
			const auto team = memory::safe_read<int>( controller + SCHEMA( "C_BaseEntity", "m_iTeamNum"_hash ) ).value_or( 0 );
			state.agent = team == 3 ? user->ct_agent : user->t_agent;
			state.skin_count = user->skins.size( );
		}
		...
		if (pawn && settings::g_misc.m_scoreboard_weapons.enabled.value) {
		...
		if (state.weapons.empty () && !state.kryptik) {
		...
		const auto script = std::format (
			R"(if(typeof(SClient)!=='undefined'){{SClient.receive({{type:"updateWeapons",content:{{xuid:"{}",account_id:"{}",weapons:{},active_path:"{}",kryptik:{},agent:{},skin_count:{}}}}});}})",
			steamid,
			account_id,
			weapons_json,
			state.active_name,
			state.kryptik ? "true" : "false",
			state.agent,
			state.skin_count
		);
