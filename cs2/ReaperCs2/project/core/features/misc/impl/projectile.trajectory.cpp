...
namespace features::misc {
	...
	void projectile_trajectory::render_trajectory( xdraw::draw_list& draw_list, const trajectory& traj, float alpha, bool is_thrown, const systems::local::snapshot& local ) const
	{
		...
		if ( cfg.bounce_markers.value )
		{
			for ( const auto& bounce : traj.bounces )
			{
				const auto screen = systems::g_view.project( bounce );
				if ( !systems::g_view.projection_valid( screen ) ) continue;
				const auto marker_alpha = static_cast<std::uint8_t>( 220.0f * alpha );
				draw_list.circle_filled( screen.x, screen.y, 4.0f, { 0, 0, 0, static_cast<std::uint8_t>( 170.0f * alpha ) }, 16 );
				draw_list.circle( screen.x, screen.y, 3.0f, { color.value.r, color.value.g, color.value.b, marker_alpha }, 16, 1.5f );
			}
		}
		...
		if ( traj.end_tick >= 0 )
		{
			...
			if ( systems::g_view.projection_valid( screen ) )
			{
				...
				if ( cfg.landing_label.value )
				{
					char landing_buf[ 40 ]{};
					std::snprintf( landing_buf, sizeof( landing_buf ), "landing  %.1fs", std::max( 0.0f, traj.duration ) );
					const auto [tw, th] = xdraw::measure_text( landing_buf );
					draw_list.text( std::floor( screen.x - tw * 0.5f ), std::floor( screen.y + 8.0f ), landing_buf, { color.value.r, color.value.g, color.value.b, det_a }, xdraw::text_style::outlined );
				}
			}
		}
	}
