...
namespace features::misc {
	...
	void impacts::on_player_hurt( std::uintptr_t event )
	{
		...
		if ( cfg.hit_marker )
		{
			...
			if ( has_position )
			{
				this->m_hitmarkers.push_back( { position, current_time, data.damage, data.hitgroup, is_kill } );
		...
		if ( is_kill )
		{
			const auto victim_name = this->get_player_name( data.victim );
			rendering::g_widgets.notify( "kill confirmed", victim_name.empty( ) ? "enemy eliminated" : victim_name + " eliminated", 3.0f );
		}
		features::misc::g_hud.on_hit( is_kill );
	...
	void impacts::render_hit_markers( xdraw::draw_list& draw_list, float time )
	{
		...
		for ( auto it = this->m_hitmarkers.begin( ); it != this->m_hitmarkers.end( ); )
		{
			...
			const auto& base_color = it->kill ? cfg.hit_marker_kill_color.value : it->hitgroup == 1 ? cfg.hit_marker_headshot_color.value : cfg.hit_marker_color.value;
			const auto color = xdraw::color( base_color.r, base_color.g, base_color.b, alpha );

			const auto x = screen.x, y = screen.y;

			const auto marker_type = cfg.hit_marker_type.value;
			const auto show_classic = marker_type == settings::misc::impacts::marker_type::classic || marker_type == settings::misc::impacts::marker_type::both;
			...
			const auto show_circle = marker_type == settings::misc::impacts::marker_type::circle;
			const auto show_cross = marker_type == settings::misc::impacts::marker_type::cross;
			...
			else if ( show_circle || show_cross )
			{
				size = 8.0f + 5.0f * ( 1.0f - std::min( elapsed * 12.0f, 1.0f ) );
			}
			...
			if ( show_damage )
			{
				...
				const auto text_y = y - base_offset - text_h * 0.5f - cfg.damage_number_rise.value * std::clamp( progress, 0.0f, 1.0f );
			...
			if ( cfg.hit_marker_glow && alpha > 0 )
			{
				...
				if ( show_damage )
				{
					glow.text( draw_x, draw_y, damage_text, xdraw::color{ base_color.r, base_color.g, base_color.b, glow_a } );
				}
			}
			...
			{
				draw_list.circle( x, y, size, color, thickness, 32 );
			}
			else if ( show_cross )
			{
				const float horizontal[ ]{ x - size, y, x + size, y };
				const float vertical[ ]{ x, y - size, x, y + size };
				draw_list.polyline( { horizontal, 4 }, color, false, thickness );
				draw_list.polyline( { vertical, 4 }, color, false, thickness );
			}
