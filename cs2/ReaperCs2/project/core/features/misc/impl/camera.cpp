...
namespace features::misc {
	...
	{
		const auto local = systems::g_local.get( );
		if ( !player_pawn || player_pawn != local.pawn )
		{
			return;
		}

		const auto sensitivity_offset = SCHEMA( "C_BasePlayerPawn", "m_flFOVSensitivityAdjust"_hash );
		const auto current_adjust = memory::read<float>( player_pawn + sensitivity_offset );
		if ( this->m_fov_sensitivity_pawn != player_pawn )
		{
			this->m_fov_sensitivity_pawn = player_pawn;
			this->m_original_fov_sensitivity = -1.0f;
			this->m_cached_fov_sensitivity = -1.0f;
			this->m_fov_sensitivity_overridden = false;
		}

		const auto& cfg = settings::g_misc.m_camera;
		if ( !cfg.change_fov.value )
		{
			if ( this->m_fov_sensitivity_overridden && this->m_original_fov_sensitivity >= 0.0f
				&& std::fabsf( current_adjust - this->m_original_fov_sensitivity ) >= 0.0001f )
			{
				memory::write<float>( player_pawn + sensitivity_offset, this->m_original_fov_sensitivity );
			}
		...
		if ( this->m_original_fov_sensitivity < 0.0f )
		{
			this->m_original_fov_sensitivity = current_adjust;
		}
		this->m_fov_sensitivity_overridden = true;
		...
		memory::write<float>( player_pawn + sensitivity_offset, desired );
	}
