...
namespace features::misc {
	...
	{
		this->m_hit_reaction = std::max( 0.0f, this->m_hit_reaction - xdraw::delta_time( ) * 4.0f );
	...
	void hud::on_hit( bool killed )
	{
		this->m_hit_reaction = 1.0f;
		this->m_hit_killed = killed;
	}
	...
	void hud::do_crosshair( xdraw::draw_list& draw_list, float cx, float cy ) const
	{
		...
		const auto reaction = cfg.hit_reaction.value ? this->m_hit_reaction : 0.0f;
		const auto s = cfg.size.value + reaction * 3.0f;
		const auto o = cfg.outline.value;
		const auto color = reaction > 0.0f ? ( this->m_hit_killed ? cfg.kill_color.value : cfg.hit_color.value ) : cfg.color.value;

		if ( cfg.type.value == settings::misc::hud::crosshair::style::dot )
		{
			if ( o > 0.0f ) draw_list.rect_filled( cx - s - o, cy - s - o, ( s + o ) * 2.0f, ( s + o ) * 2.0f, cfg.outline_color );
			draw_list.rect_filled( cx - s, cy - s, s * 2.0f, s * 2.0f, color );
		}
		else if ( cfg.type.value == settings::misc::hud::crosshair::style::circle )
		{
			draw_list.circle( cx, cy, std::max( 3.0f, s * 3.0f ), color, std::max( 1.0f, o ), 32 );
		}
		else
		{
			const auto length = std::max( 3.0f, s * 4.0f );
			const float horizontal[ ]{ cx - length, cy, cx + length, cy };
			const float vertical[ ]{ cx, cy - length, cx, cy + length };
			draw_list.polyline( { horizontal, 4 }, color, false, std::max( 1.0f, o ) );
			draw_list.polyline( { vertical, 4 }, color, false, std::max( 1.0f, o ) );
		}

		if ( reaction > 0.01f )
		{
			draw_list.circle( cx, cy, 8.0f + ( 1.0f - reaction ) * 12.0f, color.alpha( static_cast<std::uint8_t>( 180.0f * reaction ) ), 1.2f, 32 );
		}
	}
	...
	void hud::do_velocity( xdraw::draw_list& draw_list, float cx, float screen_h, std::uintptr_t local_pawn )
	{
		...
		if ( !cfg.counter.value && !cfg.chart.value && !cfg.stamina.value && !cfg.jump_state.value )
		{
			return;
		}
		...
		const auto stamina = movement_services ? std::clamp( memory::read<float>( movement_services + SCHEMA( "CCSPlayer_MovementServices", "m_flStamina"_hash ) ), 0.0f, 100.0f ) : 0.0f;
		const auto flags = memory::read<std::uint32_t>( local_pawn + SCHEMA( "C_BaseEntity", "m_fFlags"_hash ) );
		const auto on_ground = ( flags & 1u ) != 0u;
		...
		const auto status_enabled = cfg.stamina.value || cfg.jump_state.value;
		const auto status_h = status_enabled ? 22.0f : 0.0f;
		...
		const auto section_count = static_cast<int>( cfg.counter.value ) + static_cast<int>( cfg.chart.value ) + static_cast<int>( status_enabled );
		const auto stack_gap = section_count > 1 ? section_gap : 0.0f;
		const auto panel_h = counter_block_h + chart_block_h + status_h + stack_gap * std::max( 0, section_count - 1 );
		...
		if ( cfg.counter.value )
		{
			...
			content_y += counter_block_h + ( section_count > 1 ? stack_gap : 0.0f );
		}

		if ( status_enabled )
		{
			const auto status_x = panel_x + inner_pad;
			const auto status_w = panel_w - inner_pad * 2.0f;
			draw_list.rect_filled( status_x, content_y, status_w, status_h, s.child_bg, xdraw::corner_radius{ inner_r } );
			auto sx = status_x + 7.0f;
			if ( cfg.jump_state.value )
			{
				const auto state_text = on_ground ? "grounded" : ( velocity.z > 0.0f ? "rising" : "falling" );
				draw_list.text( sx, content_y + 4.0f, state_text, on_ground ? accent_dim : accent );
				sx += xdraw::measure_text( state_text ).first + 10.0f;
			}
			if ( cfg.stamina.value )
			{
				char stamina_buf[ 24 ]{};
				std::snprintf( stamina_buf, sizeof( stamina_buf ), "stamina %.0f", stamina );
				draw_list.text( sx, content_y + 4.0f, stamina_buf, stamina > 45.0f ? xdraw::color{ 255, 150, 120, accent.a } : accent_dim );
			}
			content_y += status_h + ( cfg.chart.value ? stack_gap : 0.0f );
		}
