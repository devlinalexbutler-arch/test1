...
namespace features::esp::other {
	...
	void overlay::add_spectators( xdraw::draw_list& draw_list )
	{
		...
		for ( const auto& player : systems::g_entities.get_by_type( systems::entities::type::player ) )
		{
			...
			e.observer_mode = memory::safe_read<int>( observer_services + SCHEMA( "CPlayer_ObserverServices", "m_iObserverMode"_hash ) ).value_or( 0 );
		}
		...
		auto x = std::clamp( settings::g_esp.m_other.spectator_x.value * static_cast<float>( screen_w ), 0.0f, std::max( 0.0f, static_cast<float>( screen_w ) - header_w ) );
		auto ry = std::clamp( settings::g_esp.m_other.spectator_y.value * static_cast<float>( screen_h ), 0.0f, std::max( 0.0f, static_cast<float>( screen_h ) - total_h ) );
		static bool dragging{};
		static float drag_x{}, drag_y{};
		const auto& input = xui::ctx( ).input;
		if ( rendering::g_menu.is_open( ) && input.mouse_clicked && xui::rect{ x, ry, header_w, header_h }.contains( input.mouse_x, input.mouse_y ) )
		{
			dragging = true;
			drag_x = input.mouse_x - x;
			drag_y = input.mouse_y - ry;
		}
		if ( dragging )
		{
			if ( input.mouse_down )
			{
				...
				ry = std::clamp( input.mouse_y - drag_y, 0.0f, std::max( 0.0f, static_cast<float>( screen_h ) - total_h ) );
				settings::g_esp.m_other.spectator_x = x / std::max( 1.0f, static_cast<float>( screen_w ) );
				settings::g_esp.m_other.spectator_y = ry / std::max( 1.0f, static_cast<float>( screen_h ) );
			}
			else dragging = false;
		}
		...
		for ( auto i = 0; i < count; ++i )
		{
			...
			const auto mode_text = e.observer_mode == 4 ? "first person" : e.observer_mode == 5 ? "third person" : e.observer_mode == 6 ? "free camera" : "watching";
			const auto [nw, nh] = xdraw::measure_text( e.name );
			const auto compact = settings::g_esp.m_other.spectator_compact.value;
			const auto avatar_tex = settings::g_esp.m_other.spectator_avatars.value && !compact ? avatars.get( e.steam_id ) : nullptr;
			const auto has_avatar = avatar_tex != nullptr;
			const auto [mw, mh] = xdraw::measure_text( mode_text );
			const auto show_mode = settings::g_esp.m_other.spectator_view_mode.value && !compact;

			const auto name_pill_w = nw + text_pad_x * 2.0f;
			const auto mode_pill_w = show_mode ? mw + text_pad_x * 2.0f : 0.0f;
			const auto row_w = inner_pad + name_pill_w + ( show_mode ? inner_pad + mode_pill_w : 0.0f ) + ( has_avatar ? inner_pad + avatar_pill_size : 0.0f ) + inner_pad;
			...
			{
				cx += inner_pad;
				draw_list.rect_filled( cx, ry + inner_pad, mode_pill_w, inner_h, s.child_bg, xdraw::corner_radius{ inner_r } );
				draw_list.text( cx + text_pad_x, ry + ( row_h - mh ) * 0.5f + text_nudge, mode_text, s.text_dim );
				cx += mode_pill_w;
			}
