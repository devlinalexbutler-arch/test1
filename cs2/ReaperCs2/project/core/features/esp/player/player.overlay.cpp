...
namespace features::esp::player {
	...
	void overlay::on_render( xdraw::draw_list& draw_list )
	{
		...
		for ( const auto& player : players )
		{
			...
			const auto steam_id = memory::safe_read<std::uint64_t>( info.controller + SCHEMA( "CBasePlayerController", "m_steamID"_hash ) ).value_or( 0 );
			if ( const auto user = presence::find( steam_id ) )
			{
				const auto agent = info.team == 3 ? user->ct_agent : user->t_agent;
				const auto label = std::format( "kryptik  agent {}  skins {}", agent, user->skins.size( ) );
				const auto [ tw, th ] = xdraw::measure_text( label );
				const auto x = std::floorf( bounds.min.x + bounds.width( ) * 0.5f - tw * 0.5f );
				const auto y = std::floorf( bounds.min.y - th - 14.0f - offsets.top );
				draw_list.text( x, y, label, xdraw::color( 0, 220, 90, 255 ), xdraw::text_style::outlined );
			}
		}
	}
