...
namespace features::changer {
	...
	void econ_item_system::register_custom_agents( )
	{
		struct entry
		{
			std::int16_t def;
		...
		static constexpr entry k_custom[ ]
		{
#include "custom_agents.inc"
		};

		for ( const auto& e : k_custom )
		{
			item_def def{};
			def.def_index = e.def;
			def.item_class = "custom_agent";
			def.name = e.name;
			def.localized_name = e.name;
			def.model_player = e.model;
			def.used_by_classes = 0xc; // T + CT
			def.category = item_category::agent;
			def.rarity = 6;
			this->m_item_defs.push_back( std::move( def ) );
		}
	}
