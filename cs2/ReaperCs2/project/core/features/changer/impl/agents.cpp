...
namespace features::changer {
	namespace {
		bool precache_player_model( const std::string& path )
		{
			static std::unordered_map<std::string, bool> validation_cache{};
			if ( const auto cached = validation_cache.find( path ); cached != validation_cache.end( ) )
			{
				return cached->second;
			}

			if ( path.empty( ) || !addresses::globals::resource_system )
			{
				return false;
			}
			...
			if ( !init_path_buffer || !precache_resource )
			{
				return false;
			}
			...
			struct buffer_string
			{
				...
				std::uint32_t unknown2{ 0xc00000c8 };
				...
				std::uintptr_t unknown3{};
				std::uintptr_t resource_type{};
			} buffer{};

			memory::call<void>( init_path_buffer, &buffer, path.c_str( ) );
			buffer.resource_type = 'ldmv';
			memory::call<void>(
				precache_resource,
				addresses::globals::resource_system,
				&buffer,
				"" );
			...
			const auto binding = memory::call_vfunc<std::uintptr_t>(
				addresses::globals::resource_system,
				79,
				&buffer,
				0ll );
			const auto resource = binding ? memory::safe_read<std::uintptr_t>( binding ) : std::nullopt;
			const auto valid = resource && *resource;
			validation_cache.emplace( path, valid );
			return valid;
		}
	}
	...
	void agents::on_frame_stage_notify( )
	{
		...
		this->preserve_first_person_arms( local.pawn, !this->m_overridden );
		...
		{
			if ( selected->def_index < 0 )
			{
				this->preserve_first_person_arms( local.pawn, false );
			}
			return;
		}
		...
		if ( selected->def_index < 0 &&
			...
			!precache_player_model( selected->model_player ) )
		{
			return;
		}
	...
	void agents::preserve_first_person_arms( std::uintptr_t pawn, bool capture )
	{
		const auto arms_handle = memory::read<std::uint32_t>( pawn + SCHEMA( "C_CSPlayerPawn", "m_hHudModelArms"_hash ) );
		const auto arms = arms_handle ? systems::g_entities.lookup( arms_handle ) : 0;
		if ( !arms )
		{
			return;
		}

		if ( this->m_tracked_arms != arms )
		{
			this->m_tracked_arms = arms;
			capture = this->m_original_arms_model.empty( );
		}

		const auto scene_node = memory::read<std::uintptr_t>( arms + SCHEMA( "C_BaseEntity", "m_pGameSceneNode"_hash ) );
		if ( !scene_node )
		{
			return;
		}

		const auto model_state = scene_node + SCHEMA( "CSkeletonInstance", "m_modelState"_hash );
		const auto model_name_ptr = memory::read<std::uintptr_t>( model_state + SCHEMA( "CModelState", "m_ModelName"_hash ) );
		const auto current_model = model_name_ptr ? memory::read_string( model_name_ptr ) : std::string{};

		if ( capture && this->m_original_arms_model.empty( ) && !current_model.empty( ) )
		{
			this->m_original_arms_model = current_model;
			return;
		}

		if ( !this->m_original_arms_model.empty( ) && current_model != this->m_original_arms_model )
		{
			memory::call<void>( PATTERN( patterns::set_player_model ), arms, this->m_original_arms_model.c_str( ) );
		}
	}
