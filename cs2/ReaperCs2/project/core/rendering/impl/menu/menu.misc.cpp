...
namespace rendering {
	...
	namespace detail {
		...
		constexpr const char* marker_types[ ]{ "classic", "damage", "both", "circle", "cross" };
		constexpr const char* crosshair_types[ ]{ "dot", "cross", "circle" };
	...
	void menu::draw_misc( float group_w ) const
	{
		...
		if ( subtab == 0 )
		{
			...
			if ( xui::begin_child( "##misc_impacts", col_w ) )
			{
				...
				xui::tooltip( "On-screen log of your hits (damage, hitbox, distance).\n"
				              "Right-click to set how long each line stays." );
				...
				xui::tooltip( "Print the same hit information to the game console." );

				xui::checkbox( "chat logs", impacts.chat_log );
				xui::tooltip( "Print hit information into chat (local / team)." );

				xui::checkbox( "miss logs", impacts.miss_log );
				xui::tooltip( "On-screen log when a shot misses and why.\n"
				              "Right-click to set duration." );
				...
				xui::tooltip( "Play a sound on a confirmed hit.\n"
				              "Right-click for type, volume, and custom file." );
				...
				xui::tooltip( "Draw a marker (classic / damage / both) on hit.\n"
				              "Right-click for type, duration, and color." );
				if ( xui::begin_popup( "##hitmarker_popup", 220.0f ) )
				{
					xui::combo( "type##hm", impacts.hit_marker_type.value, detail::marker_types, 5 );
					xui::tooltip( "Choose classic marker, damage number, circle, cross, or combined feedback." );

					xui::slider_float( "duration##hm", impacts.hit_marker_duration, 0.1f, 5.0f, "%.1fs" );
					xui::tooltip( "Set how long hit markers and damage numbers remain visible." );
					xui::color_picker( "color##hm", impacts.hit_marker_color );
					xui::tooltip( "Set the color used for normal hits." );
					xui::color_picker( "headshot color##hm", impacts.hit_marker_headshot_color );
					xui::tooltip( "Set the marker color used for headshots." );
					xui::color_picker( "kill color##hm", impacts.hit_marker_kill_color );
					xui::tooltip( "Set the marker color used when the hit kills the target." );
					xui::slider_float( "damage rise##hm", impacts.damage_number_rise, 0.0f, 80.0f, "%.0fpx" );
					xui::tooltip( "Set how far floating damage numbers travel upward." );
					xui::checkbox( "glow##hm", impacts.hit_marker_glow );
					xui::tooltip( "Add a soft glow around hit markers and damage numbers." );
					if ( impacts.hit_marker_glow.value )
					{
						xui::slider_float( "glow strength##hm", impacts.hit_marker_glow_strength, 0.1f, 2.0f, "%.2f" );
						xui::tooltip( "Control the brightness and spread of the hit-marker glow." );
					}
					xui::end_popup( );
				}

				xui::checkbox( "hit effect", impacts.hit_effect );
				xui::tooltip( "Screen flash / tint when you land a hit." );
				...
				xui::tooltip( "Play a sound when you confirm a kill." );
				...
				xui::tooltip( "Screen effect when you confirm a kill." );
				...
				xui::tooltip( "Show equipped weapons next to names on the scoreboard." );
				...
				xui::tooltip( "Draw overlay boxes and/or sparks at bullet impact points." );
				...
				xui::tooltip( "Draw a line from your gun to the impact point." );
			...
			if ( xui::begin_child( "##misc_visuals", col_w ) )
			{
				...
				xui::tooltip( "Predict the grenade path while holding and after throw.\n"
				              "Right-click for colors and straight-throw mode." );
				if ( xui::begin_popup( "##traj_popup", 220.0f ) )
				{
					xui::checkbox( "straight throw", traj.straight_throw );
					xui::tooltip( "Use a simplified straight launch direction for the prediction." );
					xui::checkbox( "bounce markers", traj.bounce_markers );
					xui::tooltip( "Mark every predicted surface bounce along the grenade path." );
					xui::checkbox( "landing label", traj.landing_label );
					xui::tooltip( "Label the predicted final landing point." );
				...
				xui::tooltip( "Attach a colored point light to your player." );
				...
				xui::tooltip( "Color the crosshair by whether the shot can wallbang." );
			...
			if ( xui::begin_child( "##misc_movement", col_w ) )
			{
				...
				xui::tooltip( "Automatically jump the instant you land while holding space." );

				xui::checkbox( "test strafer", mov.m_test_strafer.enabled );
				xui::tooltip( "Experimental strafer used for tuning. Leave off unless testing." );

				xui::checkbox( "auto strafe", mov.auto_strafe );
				xui::tooltip( "Air-strafe assist. Right-click for type, direction, and smoothness." );
				if ( mov.auto_strafe.value )
				{
					if ( xui::begin_popup( "##strafe_popup", 240.0f ) )
					{
						xui::slider_int( "type 0 normal 1 dir", mov.auto_strafe_type, 0, 1, "%d" );
						xui::slider_int( "dir 0 default 1 rage", mov.directional_type, 0, 1, "%d" );
						xui::slider_int( "smooth", mov.strafe_smooth, 0, 100, "%d" );
						xui::checkbox( "silent", mov.silent_strafe );
						xui::end_popup( );
					}
				}
				xui::checkbox( "jumpbug", mov.jumpbug );
				xui::tooltip( "Crouch-jump onto an edge to keep speed instead of landing." );

				xui::checkbox( "fastladder", mov.fastladder );
				xui::tooltip( "Climb ladders faster by overriding the default ladder move." );

				xui::checkbox( "edgejump", mov.edgejump );
				xui::tooltip( "Jump automatically on the last tick before you walk off an edge." );

				xui::checkbox( "edgestop", mov.edgestop );
				xui::tooltip( "Kill horizontal speed on the last tick before walking off." );

				xui::checkbox( "edgebug", mov.edgebug );
				xui::tooltip( "Stand on a falling edge to convert vertical speed into a slide.\n"
				              "Right-click for detection mode and pass count." );
				...
				xui::tooltip( "Cap walk speed while the bind is held.\n"
				              "Right-click to set the speed." );
			...
			if ( xui::begin_child( "##misc_other", col_w ) )
			{
				...
				xui::tooltip( "Show enemies on the radar regardless of teammate spotting." );

				xui::checkbox( "auto accept", m.auto_accept );
				xui::tooltip( "Automatically accept a matchmaking confirmation while queued." );

				xui::checkbox( "preserve killfeed", m.preserve_killfeed );
				xui::tooltip( "Keep killfeed lines from fading so you can read them later." );

				xui::checkbox( "disable game logs", m.disable_game_logs );
				xui::tooltip( "Suppress default game console damage / event spam." );

				xui::keybind( "menu key", m.menu_key.value );
				xui::tooltip( "Key used to open and close the menu." );

				xui::checkbox( "auto buy", ab.enabled );
				xui::tooltip( "Buy a preset loadout at freeze time.\n"
				              "Right-click for weapons, armor, kit, and nades." );
				...
				xui::tooltip( "Animate / set a custom clan tag." );

				xui::checkbox( "override name", m.m_name_changer.override_name );
				xui::tooltip( "Force a local display name. Right-click to type it." );
				...
				xui::tooltip( "Top-right info pill. Right-click to toggle fps, ping,\n"
				              "time, user, map, tickrate, and velocity." );
				if ( xui::begin_popup( "##watermark_popup", 200.0f ) )
				{
					xui::checkbox( "fps##wm",      m.m_watermark.show_fps );
					xui::tooltip( "Show the current frame rate in the watermark." );
					xui::checkbox( "ping##wm",     m.m_watermark.show_ping );
					xui::tooltip( "Show current network latency in the watermark." );
					xui::checkbox( "time##wm",     m.m_watermark.show_time );
					xui::tooltip( "Show the local clock in the watermark." );
					xui::checkbox( "user##wm",     m.m_watermark.show_user );
					xui::tooltip( "Show the current Steam display name in the watermark." );
					xui::checkbox( "map##wm",      m.m_watermark.show_map );
					xui::tooltip( "Show the current map name in the watermark." );
					xui::checkbox( "tick rate##wm",m.m_watermark.show_tick );
					xui::tooltip( "Show the estimated server tick rate in the watermark." );
					xui::checkbox( "velocity##wm", m.m_watermark.show_velocity );
					xui::tooltip( "Show current movement speed in the watermark." );
					xui::end_popup( );
				}
		...
		if ( subtab == 1 )
		{
			...
			if ( xui::begin_child( "##misc_removals", col_w ) )
			{
				...
				xui::tooltip( "Hide the default game crosshair." );
				xui::checkbox( "remove scope", rem.scope );
				xui::tooltip( "Remove the default scope overlay when zoomed." );
				xui::checkbox( "remove overhead", rem.overhead );
				xui::tooltip( "Hide player overhead info (name, icons) from the HUD." );
				xui::checkbox( "remove legs", rem.legs );
				xui::tooltip( "Hide first-person legs so they do not clip the view." );
				xui::checkbox( "remove recoil", rem.recoil );
				xui::tooltip( "Remove visual punch so the camera does not kick on spray." );
				xui::checkbox( "remove skybox fog", rem.skybox_fog );
				xui::tooltip( "Disable fog that tints the skybox." );
				xui::checkbox( "remove 3d skybox", rem.skybox_3d );
				xui::tooltip( "Disable the 3D skybox layer used on some maps." );
				xui::checkbox( "remove decals", rem.decals );
				xui::tooltip( "Stop bullet holes / blood decals from spawning." );
				xui::checkbox( "remove smoke", rem.smoke );
				xui::tooltip( "Strip smoke particles so you can see through grenades." );
				xui::slider_float( "flash alpha##flash", rem.flash_alpha, 0.0f, 100.0f, "%.0f%%" );
				xui::tooltip( "How opaque a flashbang overlay is. 0 = fully removed." );
		...
		if ( subtab == 2 )
		{
			...
			if ( xui::begin_child( "##misc_camera", col_w ) )
			{
				...
				xui::tooltip( "Override camera FOV. Right-click for hip and scoped values." );
				...
				xui::tooltip( "Move the camera behind your player. Right-click for distance." );
				...
				xui::tooltip( "Stretch or squeeze the view. 1.78 is 16:9, 1.0 is square." );
			...
			if ( xui::begin_child( "##misc_viewmodel", col_w ) )
			{
				...
				xui::tooltip( "Offset viewmodel position and FOV. Right-click for XYZ + FOV." );
		...
		{
			auto& hud = m.m_hud;
			auto& layout = m.m_hud_layout;

			if ( xui::begin_child( "##misc_hud", col_w ) )
			{
				xui::checkbox( "crosshair overlay", hud.m_crosshair.enabled );
				xui::tooltip( "Draw a custom crosshair on top of the game HUD." );
				if ( xui::begin_popup( "##xhair_popup", 220.0f ) )
				{
					xui::combo( "style##xhair", hud.m_crosshair.type.value, detail::crosshair_types, 3 );
					xui::tooltip( "Choose the shape used by the custom crosshair." );
					xui::slider_float( "size##xhair", hud.m_crosshair.size, 0.5f, 10.0f, "%.1f" );
					xui::tooltip( "Set the crosshair arm or point size." );
					xui::slider_float( "outline##xhair", hud.m_crosshair.outline, 0.0f, 4.0f, "%.1f" );
					xui::tooltip( "Set the thickness of the crosshair outline." );
					xui::color_picker( "color##xhair", hud.m_crosshair.color );
					xui::color_picker( "outline color##xhair", hud.m_crosshair.outline_color );
					xui::checkbox( "hit reaction##xhair", hud.m_crosshair.hit_reaction );
					xui::tooltip( "Temporarily recolor the crosshair when a hit or kill is registered." );
					xui::color_picker( "hit color##xhair", hud.m_crosshair.hit_color );
					xui::color_picker( "kill color##xhair", hud.m_crosshair.kill_color );
					xui::end_popup( );
				}

				xui::checkbox( "scope overlay", hud.m_scope.enabled );
				xui::tooltip( "Replace the default scope with custom lines." );
				...
				xui::tooltip( "Show current speed in units/second on the HUD." );
				xui::checkbox( "velocity chart", hud.m_velocity.chart );
				xui::tooltip( "Graph recent speed so you can read bunnyhop consistency." );
				xui::checkbox( "stamina indicator", hud.m_velocity.stamina );
				xui::tooltip( "Show movement stamina and recovery below the velocity display." );
				xui::checkbox( "jump state", hud.m_velocity.jump_state );
				xui::tooltip( "Show grounded, jumping, or airborne movement state." );
				if ( xui::begin_popup( "##velocity_hud_popup", 220.0f ) )
				{
					...
					xui::tooltip( "Move the velocity HUD upward from the bottom edge." );
					xui::slider_float( "chart width", hud.m_velocity.chart_width, 120.0f, 320.0f, "%.0f" );
					xui::tooltip( "Set the horizontal size of the velocity history chart." );
					xui::slider_float( "chart height", hud.m_velocity.chart_height, 24.0f, 80.0f, "%.0f" );
					xui::tooltip( "Set the vertical size of the velocity history chart." );
					xui::end_popup( );
				}

				xui::layout::separator( );
				xui::checkbox( "keybind panel", layout.keybinds_enabled );
				xui::tooltip( "Show a draggable list of currently active keybinds." );
				xui::checkbox( "notifications", layout.notifications_enabled );
				xui::tooltip( "Show status and event notifications in a draggable stack." );
				if ( xui::button( "reset hud positions", 160.0f, 22.0f ) )
				{
					m.m_watermark.position_x = 0.99f;
					m.m_watermark.position_y = 0.02f;
					layout.keybinds_x = 0.01f;
					layout.keybinds_y = 0.50f;
					layout.notifications_x = 0.99f;
					layout.notifications_y = 0.12f;
					rendering::g_widgets.notify( "hud layout", "widget positions reset" );
				}
				xui::tooltip( "Restore the watermark, keybind panel, notifications, and spectator list to their default positions." );
			...
			if ( xui::begin_child( "##misc_hud_hat", col_w ) )
			{
				...
				xui::tooltip( "Draw a decorative hat on your player model." );
				if ( xui::begin_popup( "##hat_popup", 220.0f ) )
				{
					xui::combo( "type##hat", hud.m_hat.type.value, detail::hat_types, 2 );
					xui::tooltip( "Choose the decorative hat model drawn on the local player." );
					xui::color_picker( "color##hat", hud.m_hat.color );
					xui::tooltip( "Set the hat's primary color." );
					xui::color_picker( "secondary color##hat", hud.m_hat.secondary_color );
					xui::tooltip( "Set the hat's secondary detail color." );
					xui::checkbox( "glow##hat", hud.m_hat.glow );
					xui::tooltip( "Add a soft glow around the decorative hat." );
					xui::slider_float( "glow strength##hat", hud.m_hat.glow_strength, 0.1f, 1.0f, "%.2f" );
					xui::tooltip( "Control the brightness of the hat glow." );
					xui::end_popup( );
				}
