	...
	void menu::draw_world( float group_w ) const
	{
		...
		if ( subtab == 0 )
		{
			...
			if ( xui::begin_child( "##esp_items", col_w ) )
			{
				...
				xui::tooltip( "Which dropped-item category these settings apply to." );

				xui::layout::separator( );

				xui::checkbox( "item esp", item.m_overlay.group_toggle( item_group ) );
				xui::tooltip( "Draw name/icon overlays on dropped items in this group.\n"
				              "Right-click for display mode, max distance, and colors." );
				...
				xui::tooltip( "Apply chams to dropped items in this group.\n"
				              "Right-click for visible / through-wall layers." );
				...
				xui::tooltip( "Engine glow on dropped items in this group.\n"
			...
			if ( xui::begin_child( "##esp_projectiles", col_w ) )
			{
				...
				xui::tooltip( "Which projectile type these settings apply to." );
				...
				xui::tooltip( "Draw overlays for this projectile type.\n"
				              "Inferno draws the fire hull; others draw name/icon." );
				...
				if ( indicator_id >= 0 )
				{
					...
					xui::tooltip( "On-screen landing / radius indicator for this nade.\n"
					              "Right-click for arc, icon, and glow colors." );
			...
			if ( xui::begin_child( "##esp_other", col_w ) )
			{
				...
				xui::tooltip( "Show planted C4 countdown on the HUD." );

				xui::checkbox( "spectator list", other.spectator_list );
				xui::tooltip( "List players currently spectating you. Right-click for indicators." );
				if ( xui::begin_popup( "##spectator_popup", 220.0f ) )
				{
					xui::checkbox( "viewing indicators", other.spectator_view_mode );
					xui::tooltip( "Show whether each spectator is watching you directly or through another player." );
					xui::checkbox( "avatars", other.spectator_avatars );
					xui::tooltip( "Display Steam avatars beside spectator names when available." );
					xui::checkbox( "compact layout", other.spectator_compact );
					xui::tooltip( "Reduce spectator-list spacing and hide larger decorative elements." );
					xui::end_popup( );
				}
		...
		if ( subtab == 1 )
		{
			...
			if ( xui::begin_child( "##world_scene_left", col_w ) )
			{
				...
				xui::tooltip( "Replace the map skybox with a custom one.\n"
				              "Right-click to pick from loaded skyboxes." );
				...
				xui::tooltip( "Tint sky, clouds, and sun independently.\n"
				              "Right-click for the three color pickers." );
				...
				xui::tooltip( "Multiply world geometry with a custom tint." );
				...
				xui::tooltip( "Override scene lighting intensity and color." );
				...
				xui::tooltip( "Override post-process bloom strength." );
				...
				xui::tooltip( "Override display gamma. Higher = brighter midtones." );
			...
			if ( xui::begin_child( "##world_scene_right", col_w ) )
			{
				...
				xui::tooltip( "Fake cinematic blur in front of / behind focus.\n"
				              "Right-click for near/far blur distances." );
		...
		if ( subtab == 2 )
		{
			...
			if ( xui::begin_child( "##world_weather", col_w ) )
			{
				...
				xui::tooltip( "Overlay snow, rain, or stars on the scene.\n"
				              "Right-click for type and particle color." );
				...
				xui::tooltip( "Add volumetric fog. Right-click for density,\n"
				              "anisotropy, draw distance, and color." );
				...
				xui::tooltip( "Make world surfaces look wet / reflective." );
				...
				xui::tooltip( "Push weather particles with a directional wind." );
