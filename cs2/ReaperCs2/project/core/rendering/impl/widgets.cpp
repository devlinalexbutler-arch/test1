...
#include <core/resources/images/kryptik_logo.hpp>
...
namespace rendering {
	...
	void widgets::draw( )
	{
		...
		if ( !announced )
		{
			this->notify( "kryptik ready", "visual overhaul loaded" );
			announced = true;
		}
		...
		if ( settings::g_misc.m_hud_layout.keybinds_enabled.value )
		{
			this->keybinds( dl );
		}
		this->notifications( dl );
	}

	void widgets::notify( std::string title, std::string message, float duration )
	{
		std::lock_guard lock( this->m_notification_mutex );
		if ( this->m_notifications.size( ) >= 8 )
		{
			this->m_notifications.erase( this->m_notifications.begin( ) );
		}
		this->m_notifications.push_back( { std::move( title ), std::move( message ), std::max( duration, 0.5f ), std::max( duration, 0.5f ) } );
	}
	...
	void widgets::watermark( xdraw::draw_list& draw_list )
	{
		...
		// ── logo ────────────────────────────────────────────────────────────
		constexpr auto logo_w = static_cast<int>( logo_icon_size );
		constexpr auto logo_h = static_cast<int>( logo_icon_size );
		static const auto logo = xdraw::load_texture( std::span<const std::byte>{
			reinterpret_cast< const std::byte* >( ::resources::images::kryptik_logo_mark ),
			sizeof( ::resources::images::kryptik_logo_mark ) } );

		const auto inner_h     = h - inner_pad * 2.0f;
		const auto logo_draw_w = static_cast<float>( logo_w );

		// ── measure text ─────────────────────────────────────────────────────
		const auto [name_tw, name_th] = xdraw::measure_text( "KryptiK" );
		const auto [user_tw, user_th] = xdraw::measure_text( "Python" );
		...
		auto x = std::clamp( settings::g_misc.m_watermark.position_x.value * static_cast<float>( screen_w ), 0.0f, std::max( 0.0f, static_cast<float>( screen_w ) - w ) );
		auto y = std::clamp( settings::g_misc.m_watermark.position_y.value * static_cast<float>( screen_h ), 0.0f, std::max( 0.0f, static_cast<float>( screen_h ) - h ) );
		static bool dragging{};
		static float drag_x{}, drag_y{};
		const auto& input = xui::ctx( ).input;
		if ( g_menu.is_open( ) && input.mouse_clicked && xui::rect{ x, y, w, h }.contains( input.mouse_x, input.mouse_y ) )
		{
			dragging = true;
			drag_x = input.mouse_x - x;
			drag_y = input.mouse_y - y;
		}
		if ( dragging )
		{
			if ( input.mouse_down )
			{
				x = std::clamp( input.mouse_x - drag_x, 0.0f, std::max( 0.0f, static_cast<float>( screen_w ) - w ) );
				y = std::clamp( input.mouse_y - drag_y, 0.0f, std::max( 0.0f, static_cast<float>( screen_h ) - h ) );
				settings::g_misc.m_watermark.position_x = x / std::max( 1.0f, static_cast<float>( screen_w ) );
				settings::g_misc.m_watermark.position_y = y / std::max( 1.0f, static_cast<float>( screen_h ) );
			}
			else dragging = false;
		}
		...
		// logo pill — hue-cycling KryptiK brand
		static float wm_logo_hue{ 0.0f };
		wm_logo_hue = std::fmod( wm_logo_hue + xdraw::delta_time( ) * 1.15f, 1.0f );
		auto wm_hsv = [ ]( float h, float sat, float val ) -> xdraw::color
		{
			const float c = val * sat;
			const float xh = c * ( 1.0f - std::fabs( std::fmod( h * 6.0f, 2.0f ) - 1.0f ) );
			const float m = val - c;
			float r{}, g{}, b{};
			switch ( static_cast< int >( h * 6.0f ) % 6 )
			{
			case 0: r = c; g = xh; b = 0; break;
			case 1: r = xh; g = c; b = 0; break;
			case 2: r = 0; g = c; b = xh; break;
			case 3: r = 0; g = xh; b = c; break;
			case 4: r = xh; g = 0; b = c; break;
			default: r = c; g = 0; b = xh; break;
			}
			return xdraw::color{
		...
		const auto logo_col = wm_hsv( wm_logo_hue, 0.85f, 1.0f );

		draw_list.rect_filled( cx, y + inner_pad, logo_pill_w, inner_h, logo_col, xdraw::corner_radius{ inner_r } );
		if ( logo )
			draw_list.image( cx + logo_icon_pad, y + ( h - static_cast<float>( logo_h ) ) * 0.5f,
				static_cast<float>( logo_w ), static_cast<float>( logo_h ), logo.Get( ), xdraw::corner_radius{ 2.0f }, xdraw::color{ 255, 255, 255, 255 } );
		draw_list.text( cx + logo_icon_pad + logo_draw_w + logo_icon_pad,
			y + ( h - name_th ) * 0.5f + text_nudge, "KryptiK", s.checkbox_mark_icon );
		cx += logo_pill_w + section_spacing;

		if ( wm.show_user.value ) draw_pill( "Python", user_tw, user_th, user_pill_w );
	...
	void widgets::keybinds( xdraw::draw_list& draw_list )
	{
		...
		for ( const auto setting : xui::binds::all( ) )
		{
			...
			if ( setting == &settings::g_combat.m_antiaim.enabled
				|| setting == &settings::g_combat.m_antiaim.override_pitch
				|| setting == &settings::g_combat.m_antiaim.override_yaw
		...
		const auto target_base_y = std::clamp( settings::g_misc.m_hud_layout.keybinds_y.value * static_cast<float>( screen_h ), 0.0f, std::max( 0.0f, static_cast<float>( screen_h ) - total_h ) );

		smoothed_base_y.set_target( target_base_y );
		smoothed_base_y.update( );

		const auto base_ry = smoothed_base_y.value( );
		auto x = std::clamp( settings::g_misc.m_hud_layout.keybinds_x.value * static_cast<float>( screen_w ), 0.0f, std::max( 0.0f, static_cast<float>( screen_w ) - 120.0f ) );
		static bool dragging{};
		static float drag_x{}, drag_y{};
		const auto& drag_input = xui::ctx( ).input;
		if ( g_menu.is_open( ) && drag_input.mouse_clicked && xui::rect{ x, base_ry, 180.0f, header_h }.contains( drag_input.mouse_x, drag_input.mouse_y ) )
		{
			dragging = true;
			drag_x = drag_input.mouse_x - x;
			drag_y = drag_input.mouse_y - base_ry;
		}
		if ( dragging )
		{
			if ( drag_input.mouse_down )
			{
				x = std::clamp( drag_input.mouse_x - drag_x, 0.0f, std::max( 0.0f, static_cast<float>( screen_w ) - 120.0f ) );
				const auto new_y = std::clamp( drag_input.mouse_y - drag_y, 0.0f, std::max( 0.0f, static_cast<float>( screen_h ) - total_h ) );
				settings::g_misc.m_hud_layout.keybinds_x = x / std::max( 1.0f, static_cast<float>( screen_w ) );
				settings::g_misc.m_hud_layout.keybinds_y = new_y / std::max( 1.0f, static_cast<float>( screen_h ) );
				smoothed_base_y.snap( new_y );
			}
			else dragging = false;
		}
	...
	void widgets::notifications( xdraw::draw_list& draw_list )
	{
		if ( !settings::g_misc.m_hud_layout.notifications_enabled.value )
		{
			return;
		}
		...
		constexpr auto width{ 280.0f };
		constexpr auto height{ 48.0f };
		constexpr auto spacing{ 6.0f };
		auto x = std::clamp( settings::g_misc.m_hud_layout.notifications_x.value * static_cast<float>( screen_w ), 0.0f, std::max( 0.0f, static_cast<float>( screen_w ) - width ) );
		auto y = std::clamp( settings::g_misc.m_hud_layout.notifications_y.value * static_cast<float>( screen_h ), 0.0f, std::max( 0.0f, static_cast<float>( screen_h ) - height ) );

		static bool dragging{};
		static float drag_x{}, drag_y{};
		const auto& input = xui::ctx( ).input;
		if ( g_menu.is_open( ) && input.mouse_clicked && xui::rect{ x, y, width, height }.contains( input.mouse_x, input.mouse_y ) )
		{
			dragging = true;
			drag_x = input.mouse_x - x;
			drag_y = input.mouse_y - y;
		...
		if ( dragging )
		{
			if ( input.mouse_down )
			{
				x = std::clamp( input.mouse_x - drag_x, 0.0f, std::max( 0.0f, static_cast<float>( screen_w ) - width ) );
				y = std::clamp( input.mouse_y - drag_y, 0.0f, std::max( 0.0f, static_cast<float>( screen_h ) - height ) );
				settings::g_misc.m_hud_layout.notifications_x = x / std::max( 1.0f, static_cast<float>( screen_w ) );
				settings::g_misc.m_hud_layout.notifications_y = y / std::max( 1.0f, static_cast<float>( screen_h ) );
			}
			else dragging = false;
		}
		...
		{
			entry.remaining -= dt;
			const auto life = std::clamp( entry.remaining / entry.duration, 0.0f, 1.0f );
			const auto intro = std::clamp( ( entry.duration - entry.remaining ) * 8.0f, 0.0f, 1.0f );
			const auto alpha_f = std::min( intro, std::min( 1.0f, life * 5.0f ) );
			const auto alpha = static_cast<std::uint8_t>( 255.0f * alpha_f );
			const auto slide = ( 1.0f - intro ) * 24.0f;
			const auto draw_x = x + slide;

			draw_list.rect_filled_blurred( draw_x, row_y, width, height, xdraw::corner_radius{ 8.0f }, xdraw::color{ 255, 255, 255, alpha } );
			draw_list.rect_filled( draw_x, row_y, width, height, s.window_bg.alpha( static_cast<std::uint8_t>( s.window_bg.a * alpha_f ) ), xdraw::corner_radius{ 8.0f } );
			draw_list.rect_filled( draw_x + 3.0f, row_y + 3.0f, 3.0f, height - 6.0f, s.accent.alpha( alpha ), xdraw::corner_radius{ 1.5f } );
			draw_list.text( draw_x + 14.0f, row_y + 8.0f, entry.title, s.accent.alpha( alpha ) );
			draw_list.text( draw_x + 14.0f, row_y + 27.0f, xui::truncate( entry.message, width - 28.0f ), s.text_dim.alpha( alpha ) );
			const auto progress = std::clamp( entry.remaining / entry.duration, 0.0f, 1.0f );
			draw_list.rect_filled( draw_x + 8.0f, row_y + height - 3.0f, ( width - 16.0f ) * progress, 1.0f, s.accent.alpha( alpha ) );
			row_y += height + spacing;
		}

		std::erase_if( this->m_notifications, [ ]( const auto& entry ) { return entry.remaining <= 0.0f; } );
	}
