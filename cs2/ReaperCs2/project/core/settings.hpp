...
namespace settings {
	...
	struct combat
	{
		struct ragebot
		{
			...
			xui::setting spinbot{ false, {}, "spinbot", "ragebot" };
			xui::setting spin_pitch_down{ false, {}, "spin pitch down", "ragebot" };
			xui::setting spin_pitch_up{ false, {}, "spin pitch up", "ragebot" };
			config::val<int> spin_speed{ 18 };
			...
			ragebot( )
			{
				this->spin_speed.reg( "ragebot", "spin speed" );
		...
		struct legitbot
		{
			...
			struct weapon_group
			{
				...
				config::val<int> response{ 65 };
				config::val<float> deadzone{ 0.03f };
				...
				xui::setting humanize{ true, {}, "humanize", "legitbot" };
				config::val<int> target_lead{ 35 };
				config::val<int> sticky{ 70 };
				...
				void init( std::string_view cat )
				{
					...
					this->humanize.category = s;

					this->fov.reg( s, "fov" );
					this->smooth.reg( s, "smooth" );
					this->response.reg( s, "response" );
					this->deadzone.reg( s, "deadzone" );
					this->hitboxes.reg( s, "hitboxes" );
					...
					this->target_lead.reg( s, "target lead" );
					this->sticky.reg( s, "sticky" );
				}
			};

			xui::setting enabled{ false, {}, "enabled", "legitbot" };
			xui::setting all_weapons{ false, {}, "all weapons", "legitbot" };
			weapon_group shared{};
			std::array<weapon_group, k_group_count> groups{};
			...
			// menu helper: which config the UI is editing right now
			weapon_group& active_group( int subtab )
			{
				if ( this->all_weapons.value )
				{
					return this->shared;
				...
				const auto idx = static_cast< std::uint32_t >( subtab );
				return this->groups[ idx < k_group_count ? idx : 2 ];
			}
		} m_legitbot{};
		...
		struct antiaim
		{
			xui::setting enabled{ false, {}, "enabled", "anti hit" };
			xui::setting override_pitch{ false, {}, "override pitch", "anti hit" };
			xui::setting override_yaw{ false, {}, "override yaw", "anti hit" };
			xui::setting at_target{ false, {}, "at target", "anti hit" };
			config::val<int> pitch_amount{ 0 };
			config::val<int> yaw_amount{ 0 };
			config::val<int> jitter_amount{ 0 };

			antiaim( )
			{
				this->pitch_amount.reg( "anti hit", "pitch amount" );
				this->yaw_amount.reg( "anti hit", "yaw amount" );
				this->jitter_amount.reg( "anti hit", "jitter amount" );
			}
		} m_antiaim{};
	...
	struct esp
	{
		...
		struct player
		{
			...
			struct chams
			{
				...
				chams_config team
				{
					...
					.primary = {.enabled = { true, {}, "visible", "chams team" }, .color = { { 120, 220, 140, 150 }, "chams team", "primary color" }, .material = { cham_ids::flat, "chams team", "primary material" } },
					.secondary = {.enabled = { true, {}, "non visible", "chams team" }, .color = { { 80, 180, 100, 118 }, "chams team", "secondary color" }, .material = { cham_ids::flat_ignorez, "chams team", "secondary material" } }
				};
		...
		struct viewmodel
		{
			chams_config weapon
			{
				...
				.primary = {.enabled = { false, {}, "primary layer", "viewmodel weapon" }, .color = { { 255, 255, 255, 255 }, "viewmodel weapon", "primary color" }, .material = { cham_ids::matte, "viewmodel weapon", "primary material" } },
				.secondary = {.enabled = { false, {}, "secondary layer", "viewmodel weapon" }, .color = { { 255, 255, 255, 255 }, "viewmodel weapon", "secondary color" }, .material = { cham_ids::matte, "viewmodel weapon", "secondary material" } },
		...
		struct other
		{
			...
			xui::setting spectator_view_mode{ true, {}, "viewing indicators", "spectator list" };
			xui::setting spectator_avatars{ true, {}, "spectator avatars", "spectator list" };
			xui::setting spectator_compact{ false, {}, "compact layout", "spectator list" };
			config::val<float> spectator_x{ 0.99f, "spectator list", "position x" };
			config::val<float> spectator_y{ 0.50f, "spectator list", "position y" };
		} m_other{};
	};
	...
	struct misc
	{
		...
		struct projectile_trajectory
		{
			...
			xui::setting bounce_markers{ true, {}, "bounce markers", "trajectory" };
			xui::setting landing_label{ true, {}, "landing label", "trajectory" };
		...
		struct impacts
		{
			...
			enum class marker_type : int { classic, damage, both, circle, cross };
			...
			config::col hit_marker_headshot_color{ { 255, 205, 95, 255 }, "impacts", "headshot color" };
			config::col hit_marker_kill_color{ { 255, 105, 105, 255 }, "impacts", "kill color" };
			config::val<float> damage_number_rise{ 32.0f, "impacts", "damage rise" };
		...
		struct hud
		{
			struct crosshair
			{
				enum class style : int { dot, cross, circle };
				xui::setting enabled{ true, {}, "crosshair overlay", "crosshair" };
				config::enm<style> type{ style::dot, "crosshair", "style" };
				...
				xui::setting hit_reaction{ true, {}, "hit reaction", "crosshair" };
				config::col hit_color{ { 255, 210, 120, 255 }, "crosshair", "hit color" };
				config::col kill_color{ { 255, 100, 100, 255 }, "crosshair", "kill color" };
			} m_crosshair{};
			...
			struct velocity
			{
				...
				xui::setting stamina{ false, {}, "stamina indicator", "velocity hud" };
				xui::setting jump_state{ false, {}, "jump state", "velocity hud" };
		...
		struct menu_theme_cfg
		{
			config::val<int> preset{ 0, "menu theme", "preset" };
			config::col accent{ { 59, 130, 246, 255 }, "menu theme", "accent" };
			config::col background{ { 16, 18, 26, 240 }, "menu theme", "background" };
			config::col panel{ { 22, 26, 36, 230 }, "menu theme", "panel" };
			config::col text{ { 236, 240, 248, 235 }, "menu theme", "text" };
			config::col text_dim{ { 130, 140, 160, 200 }, "menu theme", "text dim" };
			config::col border{ { 40, 48, 64, 120 }, "menu theme", "border" };
			config::val<float> opacity{ 100.0f, "menu theme", "opacity" };
		} m_menu_theme{};
		...
		struct watermark_cfg
		{
			...
			config::val<float> position_x{ 0.99f, "watermark", "position x" };
			config::val<float> position_y{ 0.02f, "watermark", "position y" };
		} m_watermark{};

		struct hud_layout_cfg
		{
			xui::setting keybinds_enabled{ true, {}, "keybind panel", "hud layout" };
			config::val<float> keybinds_x{ 0.01f, "hud layout", "keybinds x" };
			config::val<float> keybinds_y{ 0.50f, "hud layout", "keybinds y" };
			xui::setting notifications_enabled{ true, {}, "notifications", "hud layout" };
			config::val<float> notifications_x{ 0.99f, "hud layout", "notifications x" };
			config::val<float> notifications_y{ 0.12f, "hud layout", "notifications y" };
		} m_hud_layout{};
	...
	struct movement
	{
		...
		xui::setting auto_strafe{ false, {}, "auto strafe", "movement" };
		config::val<int> auto_strafe_type{ 0, "movement", "auto strafe type" };
		config::val<int> directional_type{ 0, "movement", "directional type" };
		config::val<int> strafe_smooth{ 70, "movement", "strafe smooth" };
		xui::setting silent_strafe{ false, {}, "silent strafe", "movement" };
	...
	struct world
	{
		...
		struct smoke_color
		{
			xui::setting enabled{ false, {}, "smoke color", "scene" };
			config::col color{ { 173, 192, 255, 200 }, "scene", "smoke color value" };
		} m_smoke{};
	};
