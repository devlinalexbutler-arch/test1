...
namespace systems {
	...
	namespace detail {
		...
		public:
			...
			template <typename T>
			void watch( std::uintptr_t address, T expected )
			{
				this->m_validators.emplace_back(
					[address, expected]
					{
						const auto current = memory::safe_read<T>( address );
			...
			void restore( )
			{
				...
				for ( const auto& validate : this->m_validators )
				{
					if ( !validate( ) )
					{
						...
						if ( !logged.exchange( true, std::memory_order_relaxed ) )
						{
							logging::console::print(
								xs( "[prediction] restore abandoned: engine object changed during simulation" ) );
						}
	...
	bool prediction::simulate( input::usercmd* cmd, const systems::local::snapshot& local, const std::function<void( )>& fn )
	{
		...
		guard.watch<std::uintptr_t>(
			local.pawn + SCHEMA( "C_BaseEntity", "m_pGameSceneNode"_hash ),
			game_scene_node );
		guard.watch<std::uintptr_t>(
			local.pawn + SCHEMA( "C_BasePlayerPawn", "m_pMovementServices"_hash ),
			movement_services );
		guard.watch<std::uintptr_t>(
			local.pawn + SCHEMA( "C_CSPlayerPawn", "m_pAimPunchServices"_hash ),
			aim_punch_services );
		guard.watch<std::uintptr_t>(
			local.pawn + SCHEMA( "C_BasePlayerPawn", "m_pWeaponServices"_hash ),
			weapon_services );
		guard.watch<std::uint32_t>(
