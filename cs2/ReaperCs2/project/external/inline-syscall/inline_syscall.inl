...
namespace jm {
    ...
    namespace detail {
        ...
        extern "C" std::int32_t jm_do_syscall(std::uint32_t id, std::uint64_t argc, const std::uint64_t* argv);

        template<class T>
        JM_INLINE_SYSCALL_FORCEINLINE std::uint64_t syscall_pack_arg(T value) noexcept
        {
        ...
        {
            return jm_do_syscall(id, 0, nullptr);
        }

        template<class... Args>
        JM_INLINE_SYSCALL_FORCEINLINE std::int32_t syscall(std::uint32_t id, Args... args) noexcept
        {
            const std::uint64_t packed[ ]{ syscall_pack_arg(args)... };
            return jm_do_syscall(id, static_cast<std::uint64_t>(sizeof...(Args)), packed);
        }
