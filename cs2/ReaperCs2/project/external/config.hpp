...
namespace config {
	...
	namespace words {
		...
		inline constexpr const char* k_wordlist[ 256 ]
		{
			...
			"someone tried to leave a 'was here'", "weener", "ReaperCs2 core1!!11", "spiderman 3", "ironman 3", "//^_^//", "wiggle", "tappy",
	...
	inline bool from_json_delta( const nlohmann::json& root )
	{
		...
		// Primary storage: %APPDATA%\KryptiK\configs\<name>.cfg (binary LZ4+json)
		// Secondary: HKCU\Software\KryptiK\configs (migrates / mirrors for compatibility)
		inline constexpr wchar_t k_root_key[ ]{ L"Software\\KryptiK\\configs" };
		inline constexpr wchar_t k_legacy_root_key[ ]{ L"Software\\ReaperCs2\\configs" };

		inline std::wstring make_name( std::wstring_view name )
		{
			return std::wstring{ name };
		}

		inline std::wstring configs_dir( )
		{
			...
			const std::wstring root = base + L"\\KryptiK";
			const std::wstring dir = root + L"\\configs";
			CreateDirectoryW( root.c_str( ), nullptr );
			CreateDirectoryW( dir.c_str( ), nullptr );
			return dir;
		}

		inline std::wstring file_path_for( std::wstring_view name )
		{
			auto dir = configs_dir( );
			...
			return dir + L'\\' + safe + L".cfg";
		}

		inline bool write_file( std::wstring_view path, const std::vector<std::uint8_t>& data )
		{
			...
			HANDLE h = CreateFileW( std::wstring( path ).c_str( ), GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr );
		...
		inline bool read_file( std::wstring_view path, std::vector<std::uint8_t>& out )
		{
			out.clear( );
			if ( path.empty( ) )
			{
			...
			HANDLE h = CreateFileW( std::wstring( path ).c_str( ), GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr );
			if ( h == INVALID_HANDLE_VALUE )
			{
				return false;
			}
			...
			out.resize( size );
			DWORD read{};
			const BOOL ok = ReadFile( h, out.data( ), size, &read, nullptr );
			CloseHandle( h );
			return ok && read == size;
		}

		inline bool reg_save( std::wstring_view name, const std::vector<std::uint8_t>& compressed )
		{
			HKEY hkey{};
			if ( RegCreateKeyExW( HKEY_CURRENT_USER, k_root_key, 0, nullptr, REG_OPTION_NON_VOLATILE, KEY_SET_VALUE, nullptr, &hkey, nullptr ) != ERROR_SUCCESS )
			{
				return false;
			}

			const auto value_name = make_name( name );
		...
		inline bool reg_load( std::wstring_view root, std::wstring_view name, std::vector<std::uint8_t>& out )
		{
			out.clear( );
			HKEY hkey{};
			if ( RegOpenKeyExW( HKEY_CURRENT_USER, std::wstring( root ).c_str( ), 0, KEY_QUERY_VALUE, &hkey ) != ERROR_SUCCESS )
			{
				return false;
			}
			...
			out.resize( size );
			if ( RegQueryValueExW( hkey, value_name.c_str( ), nullptr, nullptr, out.data( ), &size ) != ERROR_SUCCESS )
			{
				RegCloseKey( hkey );
				out.clear( );
				return false;
		...
		inline bool apply_blob( const std::vector<std::uint8_t>& buf )
		{
		...
		inline bool save( std::wstring_view name )
		{
			...
			const auto path = file_path_for( name );
			if ( !path.empty( ) && write_file( path, compressed ) )
			{
				ok = true;
			}

			// Best-effort registry mirror (does not fail the save if file succeeded)
			reg_save( name, compressed );
		...
		inline bool load( std::wstring_view name )
		{
			...
			const auto path = file_path_for( name );
			if ( !path.empty( ) && read_file( path, buf ) && apply_blob( buf ) )
			{
				return true;
			}

			if ( reg_load( k_root_key, name, buf ) && apply_blob( buf ) )
			{
				return true;
			}

			// Migrate legacy ReaperCs2 registry configs
			if ( reg_load( k_legacy_root_key, name, buf ) && apply_blob( buf ) )
			{
				// Persist into new file store so future loads are fast
				const auto path2 = file_path_for( name );
				if ( !path2.empty( ) )
				{
		...
		inline bool remove( std::wstring_view name )
		{
			...
			{
				if ( RegDeleteValueW( hkey, make_name( name ).c_str( ) ) == ERROR_SUCCESS )
				{
					ok = true;
				}
				RegCloseKey( hkey );
		...
		inline std::vector<std::wstring> list( )
		{
			...
			if ( !dir.empty( ) )
			{
				...
				if ( h != INVALID_HANDLE_VALUE )
				{
					do
					{
						...
						if ( fname.size( ) > 4 && fname.substr( fname.size( ) - 4 ) == L".cfg" )
						{
			...
			auto enum_reg = [ & ]( const wchar_t* root )
			{
				HKEY hkey{};
				if ( RegOpenKeyExW( HKEY_CURRENT_USER, root, 0, KEY_QUERY_VALUE, &hkey ) != ERROR_SUCCESS )
				{
					return;
				}
				...
				while ( true )
				{
					...
					add( std::wstring( name_buf, name_len ) );
				}
				RegCloseKey( hkey );
			};

			enum_reg( k_root_key );
			enum_reg( k_legacy_root_key );
